...
class DynamicSynapse(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, period=None, t_in_period=None, period_var=0.1, amp=1,
        ...
        #! Changed
        self.weight_oscillate_decay = weight_oscillate_decay
        self.episode_step = 500
        self.epsilon = 1e-7
        self.epsilon_0 = 1e-5
        self.epsilon_1 = 1e-5

        self.oscillate_bound = 0
    ...
    def step(self, closure=None, dt=None):
        ...
        #! ???
        # if self.episode_reward_average > 3000 and self.Change1 == True:
        #     self.a = 5e-6
        #     self.k = 0.999
        #     self.k1 = 6e-3
        #     self.b = -2e-5
        #     self.Change1 = False
        # elif self.episode_reward_average > 2000 and self.Change2 == True:
        #     self.a = 7e-6
        #     self.k = 0.996
        #     self.k2 = 1e-2
        #     self.Change2 = False
        # elif self.episode_reward_average > 1000 and self.Change3 == True:
        #     self.a = 1e-5
        #     self.k = 0.96
        #     self.k1 = 1e-1
        #     self.Change3 = False
        ...
        for group in self.param_groups:
            ...
            for i in range(len(group['params'])):
                ...
                # weight_centre += torch.where(amp > self.oscillate_bound, weight - weight_centre, weight_centre_var)
                amp += torch.where(amp > self.oscillate_bound, self.oscillate_bound - amp, 0)
                amp += torch.where(amp < self.epsilon, self.epsilon, 0)
                
                beta = - weight_oscilate_decay * modulator_amount_osci * group['lr']
                # self.epsilon_1 = np.ceil(self.episode_step / 50 * 0.25) * self.epsilon
                # if self.epsilon_1 > self.epsilon_0:
                #     self.epsilon_0 = self.epsilon_1
                ...
                #     # self.a *= np.exp(-1e-7)
                #     print('\n' + "=="*25 + " 1000 step " + "=="*25)
                #     # print('epsilon_0:', self.epsilon_0)
                #     print("a:%.9f, b:%.9f, beta:%.9f, alpha:%.9f, a + beta:%.9f, b + beta:%.9f, epsilon + beta:%.9f" 
                #           %(self.a, 
                #             self.b, 
                #             beta.cpu().detach().numpy()[-1], 
                #             alpha, 
                #             self.a + beta.cpu().detach().numpy()[-1], 
                #             self.b + beta.cpu().detach().numpy()[-1],
                #             self.epsilon + beta.cpu().detach().numpy()[-1]))
