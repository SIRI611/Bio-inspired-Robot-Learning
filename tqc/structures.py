...
class Critic_Value(Module):
    def __init__(self, state_dim, n_quantiles, n_nets):
        super().__init__()
        self.nets = []
        self.n_quantiles = n_quantiles
        self.n_nets = n_nets
        for i in range(n_nets):
            net = Mlp(state_dim, [512, 512, 512], n_quantiles)
            self.add_module(f'qf{i}', net)
            self.nets.append(net)

    def forward(self, state):
        # sa = torch.cat((state), dim=1)
        quantiles = torch.stack(tuple(net(state) for net in self.nets), dim=1)
        return quantiles
