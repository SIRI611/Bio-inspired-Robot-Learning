...
class Trainer(object):
	...
	def continue_train(self, replay_buffer, batch_size=256):
		state, action, next_state, reward, not_done = replay_buffer.sample(batch_size)
		alpha = torch.exp(self.log_alpha)
	...
	def save(self, filename):
		...
		torch.save(self.critic_value.state_dict(), filename + "_critic_value")
		torch.save(self.critic_value_target.state_dict(), filename + "_critic_value_target")
		torch.save(self.critic_value_optimizer.state_dict(), filename + "_critic_value_optimizer")
	...
	def load(self, filename):
		...
		self.critic_value.load_state_dict(torch.load(filename + "_critic_value"))
		self.critic_value_target.load_state_dict(torch.load(filename + "_critic_value_target"))
		self.critic_value_optimizer.load_state_dict(torch.load(filename + "_critic_value_optimizer"))	
