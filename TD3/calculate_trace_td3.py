...
class Config():
    def __init__(self) -> None:
        ...
        self.trace_name = "trace/actor_trace_ant_td3_1000000.0.pkl"
...
def calculate_mean_gradient(load_data, trace_num, savepath):
    ...
    layer_intermediate_0_weight = [load_data["gradient"][6*i] for i in range(trace_num)]
    layer_intermediate_0_bias = [load_data["gradient"][6*i+1] for i in range(trace_num)]
    layer_intermediate_1_weight = [load_data["gradient"][6*i+2] for i in range(trace_num)]
    layer_intermediate_1_bias = [load_data["gradient"][6*i+3] for i in range(trace_num)]
    mu_layer_weight = [load_data["gradient"][6*i+4] for i in range(trace_num)]
    mu_layer_bias = [load_data["gradient"][6*i+5] for i in range(trace_num)]
    # log_std_layer_weight = [load_data["gradient"][6*i+6] for i in range(trace_num)]
    # log_std_layer_bias = [load_data["gradient"][6*i+7] for i in range(trace_num)]
...
def calculate_abs_mean_gradient(load_data, trace_num, savepath):
    ...
    layer_intermediate_0_weight = [torch.abs(load_data["gradient"][6*i]) for i in range(trace_num)]
    layer_intermediate_0_bias = [torch.abs(load_data["gradient"][6*i+1]) for i in range(trace_num)]
    layer_intermediate_1_weight = [torch.abs(load_data["gradient"][6*i+2]) for i in range(trace_num)]
    layer_intermediate_1_bias = [torch.abs(load_data["gradient"][6*i+3]) for i in range(trace_num)]
    mu_layer_weight = [torch.abs(load_data["gradient"][6*i+4]) for i in range(trace_num)]
    mu_layer_bias = [torch.abs(load_data["gradient"][6*i+5]) for i in range(trace_num)]
    # log_std_layer_weight = [torch.abs(load_data["gradient"][6*i+6]) for i in range(trace_num)]
    # log_std_layer_bias = [torch.abs(load_data["gradient"][6*i+7]) for i in range(trace_num)]
